#include <stdio.h>
#include <string.h>
#include "./development/Game.h"

// 预设命令
void preset_command(char* command) {
    // 读取命令的第二个词
    char cmd[20];
    int cnt = sscanf(command, "%s", cmd);
    command += cnt;
    // 如果是user
    if(strcmp(cmd, "user") == 0) {
        ...
        int cellID, level;
        char player;
        sscanf(command, "%d %s %d", &cellID, player, &level);
        // 初始化地图
        // TODO: initialize map
        ...
        char user[20];
        int credit;
        sscanf(command, "%s %d", user, &credit);
        // 设置用户的积分
        ...
        char gift[20];
        int quantity;
        sscanf(command, "%s %s %d", user, gift, &quantity);
        // 给用户分配礼物
        ...
        char user[20];
        int location, days;
        sscanf(command, "%s %d %d", user, &location, &days);
        // 设置用户的位置和停留天数
...
void execute_command(char* command) {
    char cmd[20];
    int cnt = sscanf(command, "%s", cmd); // 读取命令的第一个词
    command += cnt;
